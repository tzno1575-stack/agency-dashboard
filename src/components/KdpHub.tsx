  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 md:p-6 max-w-2xl mx-auto w-full">
      ...
      <div className="flex items-center gap-2 mb-6">
        ...
        <div>
          <h2 className="text-lg font-bold text-white">KDP Hub</h2>
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        <div className="bg-[#1a1f2e] border border-[#1e293b] rounded-xl p-3 text-center">
          ...
          <p className="text-lg font-bold text-white">{books.length}</p>
          <p className="text-[10px] text-gray-600">{publishedCount} published</p>
        </div>
        <div className="bg-[#1a1f2e] border border-[#1e293b] rounded-xl p-3 text-center">
      ...
      {/* Niche Research */}
      <div className="bg-[#1a1f2e] border border-[#1e293b] rounded-xl p-4 mb-6">
        <div className="flex items-center gap-2 mb-3">
          <Search size={14} className="text-amber-400" />
          <span className="text-xs font-semibold text-gray-300">NICHE RESEARCHER</span>
        </div>
        <input
          value={nicheSearch}
          onChange={(e) => setNicheSearch(e.target.value)}
          placeholder="Search niches... (e.g., 'ADHD', 'keto', 'Islam')"
          className="w-full bg-[#0f1320] border border-[#1e293b] rounded-lg px-3 py-2 text-xs text-gray-200 placeholder-gray-600 focus:outline-none focus:border-[#3b82f6]/50 mb-3"
        />
        <div className="space-y-2">
          {filteredNiches.map((n, i) => (
            <div key={i} className="flex items-center justify-between bg-[#0f1320] rounded-lg p-2.5">
              <div>
                <p className="text-xs font-medium text-gray-300">{n.niche}</p>
      ...
      {showAddBook && (
        <div className="bg-[#1a1f2e] border border-[#1e293b] rounded-xl p-4 mb-4 space-y-3">
          ...
          <textarea placeholder="Research notes..." value={newBook.notes} onChange={e => setNewBook({...newBook, notes: e.target.value})}
            className="w-full bg-[#0f1320] border border-[#1e293b] rounded-lg px-3 py-2 text-xs text-gray-200 placeholder-gray-600 focus:outline-none focus:border-[#3b82f6]/50 resize-none h-16" />
      ...
      <div className="space-y-3">
        ...
        {books.map(book => (
          <div key={book.id} className="bg-[#1a1f2e] border border-[#1e293b] rounded-xl overflow-hidden">
              ...
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    ...
                    {book.niche && <span className="text-[10px] text-gray-600">{book.niche}</span>}
                  </div>
                </div>
              </div>
              <div className="flex items-center gap-4 shrink-0">
                {book.status === "published" || book.status === "launching" ? (
                  <div className="text-right hidden sm:block">
                    <p className="text-xs text-gray-400">{book.reviews} reviews</p>
                    <p className="text-xs font-semibold text-green-400">£{book.royalties.toFixed(2)}</p>
                  </div>
                ) : null}
                <span className="text-gray-600 text-xs">{expandedBook === book.id ? "▲" : "▼"}</span>
              </div>
            </button>

            {expandedBook === book.id && (
              <div className="px-4 pb-4 border-t border-[#1e293b] space-y-3 mt-3">
                {/* Status changer */}
                <div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {(["drafting", "editing", "published", "launching"] as KdpBook["status"][]).map(s => (
                      <button key={s} onClick={() => handleStatusChange(book.id, s)}
                        className={`text-[10px] px-2.5 py-1 rounded-full transition-colors ${book.status === s ? statusColors[s] + " font-semibold" : "bg-[#0f1320] text-gray-600 hover:text-gray-400"}`}>
                        {s}
                      </button>
                    ))}
                  </div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {book.keywords.map((kw, i) => (
                        <span key={i} className="text-[10px] bg-[#0f1320] text-gray-400 px-2 py-0.5 rounded-full">{kw}</span>
                      ))}
                    </div>
                  </div>
                )}
                ...
                {book.notes && (
                  <div>
                    ...
                    <p className="text-xs text-gray-400 bg-[#0f1320] rounded-lg p-2.5">{book.notes}</p>
                  </div>
                )}
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      <input type="number" placeholder="£0.00"
                        className="flex-1 bg-[#0f1320] border border-[#1e293b] rounded-lg px-3 py-1.5 text-xs text-gray-200 focus:outline-none focus:border-[#3b82f6]/50"
                        onChange={(e) => {
                      ...
                      <input type="number" placeholder="Reviews"
                        className="w-20 bg-[#0f1320] border border-[#1e293b] rounded-lg px-3 py-1.5 text-xs text-gray-200 focus:outline-none focus:border-[#3b82f6]/50"
                        onChange={(e) => {
        ...
        {books.length === 0 && (
          <p className="text-xs text-gray-600 text-center py-8">No books yet. Add your first KDP book above.</p>
        )}
      </div>
    </div>
  );
