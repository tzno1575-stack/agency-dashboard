          ...
          <div className="mb-4">
            <button
              ...
              className={`flex items-center gap-2 w-full px-3 py-2 rounded-lg text-xs transition-colors ${
                specMode ? "bg-indigo-500/20 border border-indigo-500/50 text-indigo-300" : "bg-[#1a1f2e] border border-[#1e293b] text-gray-500 hover:text-gray-300"
              }`}>
              <FileText size={14} />
            ...
            {specMode && (
              <div className="mt-2 space-y-1">
                {specSteps.map(step => (
                  <div key={step.id} className="bg-[#0a0e17] rounded-lg overflow-hidden">
                    <button onClick={() => toggleSpecStep(step.id)}
                      className="w-full flex items-center gap-2 px-2.5 py-2 text-left hover:bg-[#1a1f2e]/50 transition-colors">
                      ...
                      <span className={`text-[11px] flex-1 ${step.done ? "text-gray-600 line-through" : "text-gray-400"}`}>
                        {step.label}
                      </span>
                      {expandedSpec === step.id ? <ChevronDown size={10} className="text-gray-600" /> : <ChevronRight size={10} className="text-gray-600" />}
                    </button>
                    {expandedSpec === step.id && (
                      <div className="px-3 pb-2">
                        <textarea
                          ...
                          className="w-full bg-[#0f1320] border border-[#1e293b] rounded-lg p-2 text-[11px] text-gray-200 placeholder-gray-600 resize-none h-14 focus:outline-none focus:border-[#3b82f6]/50"
                        />
                      </div>
                    )}
                  </div>
